...
class CoaT_SB1(nn.Module):
    ...
    def load_weights(self, pretrained):
        print(f"SB1 loading {pretrained}")
...
class CoaT_SB2(nn.Module):
    ...
    def load_weights(self, pretrained):
        print(f"SB2 loading {pretrained}")
        checkpoint = torch.load(pretrained)
        self.load_state_dict(checkpoint['model'], strict=False)
    ...
    def forward_features(self, x1_nocls):
        B = x1_nocls.shape[0]
...
class CoaT_SB3(nn.Module):
    ...
    def load_weights(self, pretrained):
        print(f"SB3 loading {pretrained}")
        checkpoint = torch.load(pretrained)
        self.load_state_dict(checkpoint['model'], strict=False)
    ...
    def forward_features(self, x2_nocls):
        B = x2_nocls.shape[0]
...
class CoaT_SB4(nn.Module):
    ...
    def load_weights(self, pretrained):
        print(f"SB4 loading {pretrained}")
        checkpoint = torch.load(pretrained)
        self.load_state_dict(checkpoint['model'], strict=False)
    ...
    def forward_features(self, x3_nocls):
        B = x3_nocls.shape[0]
...
class CoaT(nn.Module):
    ...
    def load_checkpoint(self, checkpoint):
        self.SB1.load_weights(checkpoint)
        self.SB2.load_weights(checkpoint)
        self.SB3.load_weights(checkpoint)
        self.SB4.load_weights(checkpoint)
...
def coat_tiny(rec_enable=False, **kwargs):
    model = CoaT(patch_size=4, embed_dims=[152, 152, 152, 152], serial_depths=[2, 2, 2, 2], parallel_depth=6, num_heads=8, mlp_ratios=[4, 4, 4, 4], rec_enable=rec_enable, **kwargs)
    model.default_cfg = _cfg_coat()
    return model

@register_model
def coat_mini(rec_enable=False, **kwargs):
    model = CoaT(patch_size=4, embed_dims=[152, 216, 216, 216], serial_depths=[2, 2, 2, 2], parallel_depth=6, num_heads=8, mlp_ratios=[4, 4, 4, 4], rec_enable=rec_enable, **kwargs)
    model.default_cfg = _cfg_coat()
    return model

@register_model
def coat_small(rec_enable=False, **kwargs):
    model = CoaT(patch_size=4, embed_dims=[152, 320, 320, 320], serial_depths=[2, 2, 2, 2], parallel_depth=6, num_heads=8, mlp_ratios=[4, 4, 4, 4], rec_enable=rec_enable, **kwargs)
    model.default_cfg = _cfg_coat()
    return model
...
def coat_lite_tiny(rec_enable=False, **kwargs):
    model = CoaT(patch_size=4, embed_dims=[64, 128, 256, 320], serial_depths=[2, 2, 2, 2], parallel_depth=0, num_heads=8, mlp_ratios=[8, 8, 4, 4], rec_enable=rec_enable, **kwargs)
    model.default_cfg = _cfg_coat()
    return model

@register_model
def coat_lite_mini(rec_enable=False, **kwargs):
    model = CoaT(patch_size=4, embed_dims=[64, 128, 320, 512], serial_depths=[2, 2, 2, 2], parallel_depth=0, num_heads=8, mlp_ratios=[8, 8, 4, 4], rec_enable=rec_enable, **kwargs)
    model.default_cfg = _cfg_coat()
    return model

@register_model
def coat_lite_small(rec_enable=False, **kwargs):
    model = CoaT(patch_size=4, embed_dims=[64, 128, 320, 512], serial_depths=[3, 4, 6, 3], parallel_depth=0, num_heads=8, mlp_ratios=[8, 8, 4, 4], rec_enable=rec_enable, **kwargs)
    model.default_cfg = _cfg_coat()
    return model

@register_model
def coat_lite_medium(rec_enable=False, **kwargs):
    model = CoaT(patch_size=4, embed_dims=[128, 256, 320, 512], serial_depths=[3, 6, 10, 8], parallel_depth=0, num_heads=8, mlp_ratios=[4, 4, 4, 4], rec_enable=rec_enable, **kwargs)
    model.default_cfg = _cfg_coat()
    return model

def SB1_call(patch_size=16, in_chans=3, num_classes=1000, embed_dims=[0, 0, 0, 0], 
                 serial_depths=[0, 0, 0, 0], parallel_depth=0,
                 num_heads=0, mlp_ratios=[0, 0, 0, 0], qkv_bias=True, qk_scale=None, drop_rate=0., attn_drop_rate=0.,
                 drop_path_rate=0., norm_layer=partial(nn.LayerNorm, eps=1e-6),
                 return_interm_layers=False, out_features=None, crpe_window={3:2, 5:3, 7:3},
                 checkpoint=None,
                 **kwargs):
    model = CoaT_SB1(patch_size=patch_size, embed_dims=embed_dims, serial_depths=serial_depths, parallel_depth=parallel_depth, num_heads=num_heads, mlp_ratios=mlp_ratios, **kwargs)
    return model

def SB2_call(patch_size=16, in_chans=3, num_classes=1000, embed_dims=[0, 0, 0, 0], 
                 serial_depths=[0, 0, 0, 0], parallel_depth=0,
                 num_heads=0, mlp_ratios=[0, 0, 0, 0], qkv_bias=True, qk_scale=None, drop_rate=0., attn_drop_rate=0.,
                 drop_path_rate=0., norm_layer=partial(nn.LayerNorm, eps=1e-6),
                 return_interm_layers=False, out_features=None, crpe_window={3:2, 5:3, 7:3},
                 checkpoint=None,
                 **kwargs):
    model = CoaT_SB2(patch_size=patch_size, embed_dims=embed_dims, serial_depths=serial_depths, parallel_depth=parallel_depth, num_heads=num_heads, mlp_ratios=mlp_ratios, **kwargs)
    return model

def SB3_call(patch_size=16, in_chans=3, num_classes=1000, embed_dims=[0, 0, 0, 0], 
                 serial_depths=[0, 0, 0, 0], parallel_depth=0,
                 num_heads=0, mlp_ratios=[0, 0, 0, 0], qkv_bias=True, qk_scale=None, drop_rate=0., attn_drop_rate=0.,
                 drop_path_rate=0., norm_layer=partial(nn.LayerNorm, eps=1e-6),
                 return_interm_layers=False, out_features=None, crpe_window={3:2, 5:3, 7:3},
                 checkpoint=None,
                 **kwargs):
    model = CoaT_SB3(patch_size=patch_size, embed_dims=embed_dims, serial_depths=serial_depths, parallel_depth=parallel_depth, num_heads=num_heads, mlp_ratios=mlp_ratios, **kwargs)
    return model

def SB4_call(patch_size=16, in_chans=3, num_classes=1000, embed_dims=[0, 0, 0, 0], 
                 serial_depths=[0, 0, 0, 0], parallel_depth=0,
                 num_heads=0, mlp_ratios=[0, 0, 0, 0], qkv_bias=True, qk_scale=None, drop_rate=0., attn_drop_rate=0.,
                 drop_path_rate=0., norm_layer=partial(nn.LayerNorm, eps=1e-6),
                 return_interm_layers=False, out_features=None, crpe_window={3:2, 5:3, 7:3},
                 checkpoint=None,
                 **kwargs):
    model = CoaT_SB4(patch_size=patch_size, embed_dims=embed_dims, serial_depths=serial_depths, parallel_depth=parallel_depth, num_heads=num_heads, mlp_ratios=mlp_ratios, **kwargs)
    return model
